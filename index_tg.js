...
const PORT = process.env.SERVER_PORT || process.env.PORT || 3000; // 服务端口
...
const CFIP = process.env.CFIP || 'cdns.doon.eu.org';  // 优选域名或IP
const CFPORT = process.env.CFPORT || 443;             // 优选端口
const NAME = process.env.NAME || '';                  // 节点名称
const CHAT_ID = process.env.CHAT_ID || '';           // TG chat_id
const BOT_TOKEN = process.env.BOT_TOKEN || '';        // TG bot_token
...
async function sendTelegram(message) {
  if (!BOT_TOKEN || !CHAT_ID) {
    console.log("TG variable is empty, skip sent");
    return;
  }
