// 精简版 index_vm.js (移除 komari-agent)
...
// 从环境变量中获取 XRAY_VERSION 和 CLOUDFLARED_VERSION
...
const webPath = path.join(FILE_PATH, webName); // Xray 路径
const botPath = path.join(FILE_PATH, botName); // Cloudflared 路径
...
async function downloadAndRun() {
  ...
  for (const f of filesToDownload) {
    ...
    try {
        ...
        console.error(`Error during download or setup for ${f.file}:`, e);
        // 如果下载失败，尝试清理残留文件并抛出错误
        if (fs.existsSync(downloadPath)) fs.unlinkSync(downloadPath);
        throw new Error(`Failed to set up core file: ${f.file}`);
    }
  }
...
async function extractDomains() {
  ...
  if (!domain) {
    ...
    // 启动 cloudflared 可能需要时间，等待 3s 确保日志文件已生成
  ...
  // 生成 VMESS 链接并 Base64 编码两次作为订阅内容
  const vmessLink = `vmess://${Buffer.from(JSON.stringify(vmess)).toString('base64')}`;
  const content = Buffer.from(vmessLink).toString('base64');
...
async function pushTelegram() {
  ...
  // Telegram 消息内容是 Base64 两次编码前的 VMESS 链接
  const encodedContent = fs.readFileSync(subPath, 'utf-8');
  // 订阅内容 (Base64 twice) -> VMESS Link (Base64 once)
  const vmessLink = Buffer.from(encodedContent, 'base64').toString('utf-8'); 
